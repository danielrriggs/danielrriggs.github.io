...
**5/23**
**AWS Artifact**
- AWS Network Firewall vs AWS Firewall Manager vs GuardDuty vs Inspector vs AWS Traffic Mirroring
	- Guard duty vs inspector: Inspector for vulnerability management. GuardDuty for threat detection.
- ^^ ... GuardDuty vs WAF (for SQL injection)
- Elastic Beanstalk vs ECS
- SWF
- AWS Data sync vs Storage Gateway
- Amazon EMR, EMR serverless
- Amazon Data Lifecycle Manager (DLM)
- AWS Batch
- Static any cast ip ... Global accelerator
- Elastic fabric adapter
- AWS config vs trusted advisor
- Lambda IPs .. ENIs .. vpc subnets .. scaling
- ASG launch template,target group
- Lambda, KMS execution role. I AM policy to both sides.
- RDS storage auto scaling... Running out of disk.
- Lambda .. container image support (disk allocation )
- AWS cost explorer vs AWS budgets.
- Know what iam roles are attached to.
- What SCPs are attached to.
- AWS systems manager parameter store vs secrets manager.
- Firehouse vs kineses vs sqs vs sns. Firehouse doesn't guarantee order. Kinesis does.
- Ena vs efa
- Ec2 io1 .. 50:1 GB:IOPS
- AWS app migration service
- AWS Health (AWS Personal Health Dashboard) vs AWS Service Health Dashboard.
- Redshift Spectrum
- Athena; Athena Federated Query
- **SSE-KMS** vs **SSE-S3** - KMS provides audit trail. S3 auto rotates.
- ELB access logs feature vs AWS X-Ray ... CloudWatch App Insights vs AWS CloudTrail Lake vs CloudWatch ServiceLens vs CloudWatch Container Insights
- Run Command (**AWS Systems Manager Run Command**)
- EKS .. Karpenter vs Cluster Autoscaler .. Kubernetes Metrics Server, Horizontal Pod Autoscaling
- AWS Trusted Advisor
- AWS Wavelength
- (EC2) Enable Enhanced Networking with Elastic Fabric Adapter vs Enable Enhanced Networking with Elastic Network Adapter
- AppSync pipeline resolvers
- Aurora failover - only if using a replica, it flips the CNAME record (NOT A record); If you are running Aurora Serverless and the DB instance or AZ becomes unavailable, Aurora will automatically recreate the DB instance in a different AZ. If you do not have an Amazon Aurora Replica (i.e., single instance) and are not running Aurora Serverless, Aurora will attempt to create a new DB Instance in the same Availability Zone as the original instance.
- Use tags + IAM policies to make sure testers can use UAT resources but not PROD resources.
- A record vs CNAME - to point R53 zone apex DNS to ALB, use A record aliased to  LB DNS name.
- FSX for Windows File System vs FSX for Lustre.
- Storage Gateway vs Data sync - storage Gateway used for integration. DS more for moving.
- Management Events vs Data Events - control plane vs Data plane.
- IAM policy vs access keys .. access keys needed to make API calls.
- HA with cloudfront .. you need 2 origin groups .. ALB can't be an origin group. For CF failover.
- ALB's can't scale based on **target group** cpu utilization.
- Lex - speech to text, chatbots. Comprehend - NLP for text (not for chatbots). Poly - text to speech; not for conversational chatbots. Rekognition - object recognition.
- AWS Elastic Disaster Recovery.
- EBS encryption
- Amazon Pinpoint vs SNS
- Lambda Function URL vs API Gateway ..
- 2 ways to alert (30 days, for example) before ACM cert renewal: 1) EventBridge checks `DaysToExpiry` metric of all ACM certs in CloudWatch. 2) EventBridge checks AWS Health or ACM expiration events.
- Remote Desktop connection (RDP) .. server listens on TCP port 3389 and UDP port 3389.
- S3-IA and S3 One-Zone-IA - have 30 day minimum retention.
- Use ALB to implement sticky sessions.
- **CloudTrail** has a Log file validation feature to ensure log files have't been tampered with.
- There is a constraint in S3 that objects must be stored at least 30 days in the current storage class before you can transition them (to other storage classes).
- NLBs and ALBs can both perform HTTP health checks (on EC2 instances running within in ASG).
- Controlling Lambda permissions - applying permissions to **function’s execution role** vs the function’s **resource-based policy** - 1) execution roles grant Lambda functions access to other AWS services. You **can’t** use it to control **which entity** can invoke the function. 2) IAM roles, in general, do not support the **principal element**.
- AWS Backup .. Vault Locks .. Compliance mode vs Governance mode .. governance mode allows privileged users to potentially modify or remove backups after setting them up. Compliance mode can't be adjusted once in place.
- Data transferred between Amazon EC2, Amazon RDS, Amazon Redshift, Amazon ElastiCache instances, and Elastic Network Interfaces in the **same Availability Zone** is free (NOT the same region). Even if EC2 instances are deployed in the same Region, they could still be charged with inter-Availability Zone data transfers if the instances are distributed across different availability zones.
- KMS .. AWS managed keys don't allow for fine-grained control of key rotation. Only to enable/disable it.
- Direct Connect is meant for long-term data integration between Cloud & on-prem, not for one-off data transfers.
- AWS Application Migration Service (MGN) vs AWS Database Migration Service (DMS) vs AWS DataSync.
- You cannot directly migrate RabbitMQ to Amazon SQS (but you can migrate to Amazon MQ).
- How to find specific errors in CloudTrail logs .. Amazon QuickSight vs Amazon Athena vs **AWS CloudTrail Lake** vs AWS Security Hub.
- **Amazon Aurora Provisioned DB cluster with burstable performance DB instance class types** is not suitable for intermittent, sporadic, and unpredictable transactional workloads. This model works well when the database workload is predictable because you can adjust capacity manually based on the expected workload.
- AWS Glue DataBrew vs EMR vs Glue Studio vs Athena
	- DataBrew - no-code tool. clean, normalize, and transform data. Prep data for ML.
- AWS Systems Manager Automation runbook vs AWS Config rule
	- AWS Config rules cannot directly prevent actions. Used for monitoring and compliance checks, not intercepting AWS service operations.
	- AWS Systems Manager Automation can respond to an EventBridge event.

---
7/5
Timed test 2, questions to review - 
9
11
14
18
